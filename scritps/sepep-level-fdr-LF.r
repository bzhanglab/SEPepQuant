...
if (length(argv) != 2) {
  cat("Usage: identify-sepep-LF input_dir SEPEP_fdr\n")
  q(status = 1)
}
SEPEP_fdr=as.numeric(argv[2])

peptide = read.table(paste(argv[1],"/identify-sepep-and-quantified-by-sum.txt",sep=""),header = TRUE,stringsAsFactors=FALSE, sep = "\t",check.name=FALSE)
