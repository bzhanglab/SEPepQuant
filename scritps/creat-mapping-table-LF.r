...
if (length(argv) != 2) {
  cat("Usage:creat-sepep-mapping-table-LF input_dir output_dir\n")
  q(status = 1)
}
...
whole_mapping_table=whole_mapping_table[,c("SEPEP_label","number_of_mapped_protein","number_of_mapped_gene","protein_number_of_gene","sorted_gene_list","sorted_protein_list")]
...
for(i in 1:length(sample_list))
{
  ...
  combined_table=cbind(peptide,sub_mapping_table)
  combined_table=combined_table[,-ncol(combined_table)]
